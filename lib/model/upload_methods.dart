...
class UploadMethods {
  ...
  static UploadStatusEnum validateCSVRowContent(List<dynamic> inputRow) {
    ...
    //Position 0: Status
    validationResults[0] =
        WatchDataValidationFacade.validateStatus(inputRow[0]);

    //Position 1: Manufacturer
    validationResults[1] =
        WatchDataValidationFacade.validateManufacturer(inputRow[1]);
    ...
    validationResults[3] =
        WatchDataValidationFacade.validateSerialNumber(inputRow[3]);

    //Position 4: Reference Number
    validationResults[4] =
        WatchDataValidationFacade.validateReferenceNumber(inputRow[4]);

    //Position 5: Warranty Expiry Date
    validationResults[5] =
        WatchDataValidationFacade.validateWarrantyExpiryDate(inputRow[5]);

    //Position 6: Last Serviced Date
    validationResults[6] =
        WatchDataValidationFacade.validateLastServicedDate(inputRow[6]);

    //Position 7: Purchase Date
    validationResults[7] =
        WatchDataValidationFacade.validatePurchasedDate(inputRow[7]);

    //Position 8: Purchase Price
    validationResults[8] = WatchDataValidationFacade.validatePurchasePrice(
        inputRow[8]); //? true : inputRow[8].isDouble; //Handle case where decimals have been included - //TODO: TEST THIS!

    //Position 9: Purchased From
    validationResults[9] =
        WatchDataValidationFacade.validatePurchasedFrom(inputRow[9]);

    //Position 10: Sold Date
    validationResults[10] =
        WatchDataValidationFacade.validateSoldDate(inputRow[10]);

    //Position 11: Sold Price
    validationResults[11] = WatchDataValidationFacade.validateSoldPrice(
        inputRow[11]); // ? true : inputRow[11].isDouble;

    //Position 12: Sold To
    validationResults[12] =
        WatchDataValidationFacade.validateSoldTo(inputRow[12]);

    //Position 13: Case Diameter
    validationResults[13] =
        WatchDataValidationFacade.validateCaseDiameter(inputRow[13]);

    //Position 14: Case Thickness
    validationResults[14] =
        WatchDataValidationFacade.validateCaseThickness(inputRow[14]);

    //Position 15: Lug Width
    validationResults[15] =
        WatchDataValidationFacade.validateLugWidth(inputRow[15]);

    //Position 16: Lug to Lug
    validationResults[16] =
        WatchDataValidationFacade.validateLug2Lug(inputRow[16]);

    //Position 17: Water Resistance
    validationResults[17] =
        WatchDataValidationFacade.validateWaterResistance(inputRow[17]);
    ...
    if (result != UploadStatusEnum.fail) {
      if (WatchMethods.checkForDuplicate(
          inputRow[1].toString(), inputRow[2].toString())) {
